...
class DiffusionLoss(nn.Module):
    ...
    def prepare_text_embeddings(self):
        ...
        self.text_z = self.model.get_text_embeds([self.args.text_prompt + self.pad_prompt] * self.args.num_aug_clip, [self.args.text_negative_prompt] * self.args.num_aug_clip)
    ...
    def prepare_image_control(self):
        ...
        if background_path is not None:
            self.background = Image.open(background_path).convert("RGB").resize((512, 512))
            self.background = ImageOps.invert(self.background)
